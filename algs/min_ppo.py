...
from policies.min_actor_critic import RolloutBuffer, ActorCritic
import time
...
class PPO:
    ...
    def update(self):
        ...
        for k in range(self.K_epochs):
            ...
            old_states, old_actions, rewards, old_action_idxs, old_logprobs = self.buffer.get_torch_data(self.gamma, self.batch_size)
            if len(old_states) == 0:
                # No signal available
                self.buffer.clear()
                return
...
def rollout(env, agent, param, i_episode, testing=False, visualize=False):
    ...
    for t in range(1, param['ppo']['T']):  # Don't infinite loop while learning
        ...
        # tic = time.time()
        agent.buffer.add_experience(state['mdp'], action, reward, next_state['mdp'], log_prob)
