...
class PPO:
    def __init__(self, state_dim, action_dim, gamma, param):
        ...
        self.optimizer = torch.optim.Adam([
                        ...
                        {'params': self.rho.rho.parameters(), 'lr': lr_rho},
                    ])
    ...
    def update(self):
        ...
        for _ in range(self.K_epochs):
            ...
            surr2 = torch.clamp(ratios[:-1], 1-self.eps_clip, 1+self.eps_clip) * advantages

            # final loss of clipped objective PPO
            policy_grad = -torch.min(surr1, surr2) 
            val_loss = 0.5*self.MseLoss(state_values[:-1], rewards[:-1]) 
            entropy_loss = - 0.01*dist_entropy[:-1] 
            loss = rho_losses + policy_grad #+ val_loss + entropy_loss 
...
def run_ppo_ltl(param, env, second_order = False):
    
    agent = PPO(env.observation_space, env.action_space['mdp'].shape[0], param['gamma'], param)
